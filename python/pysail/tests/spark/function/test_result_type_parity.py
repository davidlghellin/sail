...
REFERENCE = pathlib.Path(__file__).parent / "data" / "function_result_types.json"
CATALOGUE = (
    pathlib.Path(__file__).parents[5] / "crates" / "sail-plan" / "data" / "functions" / "scalar"
)
BATCH = 40
ERROR = "<error>"
...
def catalogue_expressions() -> list[str]:
    ...
    for path in sorted(CATALOGUE.glob("*.yaml")):
        for match in re.finditer(r">\s*(SELECT [^;]+);", path.read_text()):
            ...
            if len(query) < 160 and "TABLE" not in query.upper():
...
def measure(spark, expressions: list[str]) -> dict[str, str]:
    ...
    for start in range(0, len(expressions), BATCH):
        ...
        try:
            row = spark.sql(f"SELECT {columns}").collect()[0]  # noqa: S608
            measured.update(dict(zip(chunk, list(row), strict=True)))
        except Exception:  # noqa: BLE001
            for e in chunk:
                try:
                    measured[e] = spark.sql(f"SELECT typeof({e}) AS c").collect()[0][0]  # noqa: S608
                except Exception:  # noqa: BLE001
                    measured[e] = ERROR
    return measured
...
def test_the_result_type_of_every_function_matches_the_reference(spark):
    ...
    assert not added, (
        f"{len(added)} catalogue examples are not in the reference; regenerate it:\n"
        + "\n".join(f"  {e}" for e in added[:20])
    )


def test_the_recorded_divergences_are_the_known_ones(spark):
    ...
    sail_refuses = [
        e for e, v in reference.items() if v["sail"] == ERROR and v["spark"] != ERROR
    ]

    assert len(different_type) == 51, (
        f"{len(different_type)} functions type their result differently from Spark, not 51. "
    ...
    assert len(sail_refuses) == 83, (
        f"{len(sail_refuses)} catalogue examples are answered by Spark and refused by Sail, not "
        "83. If it grew, the change under review took a working query away."
    )
...
if __name__ == "__main__":
    ...
    for expression, value in result.items():
        data.setdefault(expression, {}).update({engine_key: value})
    REFERENCE.parent.mkdir(parents=True, exist_ok=True)
    REFERENCE.write_text(json.dumps(dict(sorted(data.items())), indent=1) + "\n")
