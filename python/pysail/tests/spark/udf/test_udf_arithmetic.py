...
def test_udf_result_in_arithmetic(spark):
    ...
    for op, expected in [("+", 4), ("-", 2), ("*", 3), ("%", 0)]:
        assert spark.sql(f"SELECT plus_one(2) {op} 1 AS r").collect()[0][0] == expected
    assert spark.sql("SELECT plus_one(2) / 1 AS r").collect()[0][0] == 3.0
    assert spark.sql("SELECT plus_one(2) + plus_one(2) AS r").collect()[0][0] == 6
...
def test_udf_consuming_udt_in_arithmetic(spark):
    ...
    spark.createDataFrame(
        data=[], schema=StructType().add("a", UnnamedPythonUDT())
    ).createOrReplaceTempView("udf_arithmetic_udt")
