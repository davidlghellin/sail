    ...
    match expr {
        ...
        spec::Expr::UnresolvedFunction(mut function) => {
            ...
            function.named_arguments = function
                .named_arguments
                .into_iter()
                .map(|(name, expr)| (name, expr_with_default_column_values(expr)))
                .collect();
            function.filter = function
                .filter
                .map(|expr| Box::new(expr_with_default_column_values(*expr)));
            function.order_by = function.order_by.map(|order_by| {
                order_by
                    .into_iter()
                    .map(sort_order_with_default_column_values)
                    .collect()
            });
        ...
        spec::Expr::SortOrder(sort) => {
            spec::Expr::SortOrder(sort_order_with_default_column_values(sort))
        }
        spec::Expr::LambdaFunction {
            function,
            arguments,
        } => spec::Expr::LambdaFunction {
            function: Box::new(expr_with_default_column_values(*function)),
            arguments,
        },
        spec::Expr::Window {
            window_function,
            ...
            window_function: Box::new(expr_with_default_column_values(*window_function)),
            window,
        },
        spec::Expr::UnresolvedExtractValue { child, extraction } => {
            spec::Expr::UnresolvedExtractValue {
                child: Box::new(expr_with_default_column_values(*child)),
                extraction: Box::new(expr_with_default_column_values(*extraction)),
            }
        }
        spec::Expr::UpdateFields {
            ...
            struct_expression: Box::new(expr_with_default_column_values(*struct_expression)),
            field_name,
        ...
        spec::Expr::NamedArgument { key, value } => spec::Expr::NamedArgument {
            key,
            value: Box::new(expr_with_default_column_values(*value)),
        },
...
fn sort_order_with_default_column_values(sort: spec::SortOrder) -> spec::SortOrder {
    let spec::SortOrder {
        child,
        direction,
        null_ordering,
