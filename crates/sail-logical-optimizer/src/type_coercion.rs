...
use datafusion_common::{
    exec_err, internal_datafusion_err, internal_err, not_impl_err, plan_datafusion_err, plan_err,
...
use datafusion_expr::type_coercion::functions::{
    fields_with_udf, value_fields_with_higher_order_udf_and_lambdas, UDFCoercionExt,
};
...
use datafusion_expr::{
    is_false, is_not_false, is_not_true, is_not_unknown, is_true, is_unknown, lit, not, Cast, Expr,
    ExprSchemable, Join, Limit, LogicalPlan, Operator, Projection, Union, ValueOrLambda,
...
mod test {
    ...
    use datafusion::arrow::datatypes::{DataType, Field, Schema, SchemaBuilder, TimeUnit};
    use datafusion::functions_aggregate::average::AvgAccumulator;
    use datafusion::optimizer::Analyzer;
    use insta::assert_snapshot;

    use crate::type_coercion::{coerce_case_expression, TypeCoercion, TypeCoercionRewriter};
    use datafusion_common::config::ConfigOptions;
    ...
    use datafusion_expr::{
        cast, col, create_udaf, is_true, lit, AccumulatorFactoryFunction, AggregateUDF, BinaryExpr,
        Case, ColumnarValue, Expr, ExprSchemable, Filter, LogicalPlan, Operator,
        ScalarFunctionArgs, ScalarUDF, ScalarUDFImpl, Signature, SimpleAggregateUDF, Subquery,
        Union, Volatility,
    };
    ...
    fn assert_type_coercion_error(plan: LogicalPlan, expected_substr: &str) -> Result<()> {
        let options = ConfigOptions::default();
    ...
    fn coerce_utf8view_output() -> Result<()> {
        ...
        let plan =
            LogicalPlan::Projection(Projection::try_new(vec![expr.clone()], Arc::clone(&empty))?);
        ...
        let bool_plan =
            LogicalPlan::Projection(Projection::try_new(vec![bool_expr], Arc::clone(&empty))?);
        // Plan B: no coerce
        coerce_on_output_if_viewtype!(
            false,
            bool_plan.clone(),
        ...
        let plan =
            LogicalPlan::Projection(Projection::try_new(vec![col("a")], Arc::new(sort_plan))?);
        // Plan D: no coerce
        coerce_on_output_if_viewtype!(
            false,
            plan.clone(),
    ...
    fn nested_case() -> Result<()> {
        ...
        let plan =
            LogicalPlan::Projection(Projection::try_new(vec![expr.clone().or(expr)], empty)?);
    ...
    fn scalar_function() -> Result<()> {
        ...
        let plan = LogicalPlan::Projection(Projection::try_new(vec![scalar_function_expr], empty)?);
    ...
    fn binary_op_date32_op_interval() -> Result<()> {
        ...
        let expr =
            cast(lit("1998-03-18"), DataType::Date32) + lit(ScalarValue::new_interval_dt(123, 456));
        let empty = empty();
        let plan = LogicalPlan::Projection(Projection::try_new(vec![expr], empty)?);
    ...
    fn between_case() -> Result<()> {
        let expr = col("a").between(
            ...
            cast(lit("2002-05-08"), DataType::Date32) + lit(ScalarValue::new_interval_ym(0, 1)),
        );
    ...
    fn is_bool_for_type_coercion() -> Result<()> {
        ...
        let plan = LogicalPlan::Projection(Projection::try_new(vec![expr.clone()], empty)?);
    ...
    fn concat_for_type_coercion() -> Result<()> {
        ...
        .call(args.to_vec());
        let plan = LogicalPlan::Projection(Projection::try_new(vec![expr], Arc::clone(&empty))?);
        assert_analyzed_plan_eq!(
            plan,
            @r#"
    ...
    macro_rules! test_case_expression {
        ($expr:expr, $when_then:expr, $case_when_type:expr, $then_else_type:expr, $schema:expr) => {
            ...
            let expected = cast_helper(case.clone(), &$case_when_type, &$then_else_type, &$schema);
    ...
    fn test_map_with_diff_name() -> Result<()> {
        ...
        let fields = Field::new("entries", DataType::Struct(struct_fields.clone()), false);
